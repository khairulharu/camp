...
func (u *userRepository) FindByID(ctx context.Context, userID int64) (domain.User, error) {
	...
	err := u.db.WithContext(ctx).Table("user").Where("id = ?", userID).First(user).Error
	if err != nil {
...
func (u *userRepository) FindByUsername(ctx context.Context, username string) (domain.User, error) {
	var user domain.User
	err := u.db.WithContext(ctx).Table("user").Where("name = ?", username).First(user).Error
	if err != nil {
		return domain.User{}, err
	}
	return user, nil
...
func (u *userRepository) Insert(ctx context.Context, user *domain.User) error {
	return u.db.WithContext(ctx).Table("user").Create(user).Error
}

// UpdateUser implements domain.UserRepository.
func (u *userRepository) UpdateUser(ctx context.Context, userID int64, updates map[string]interface{}) error {
	db := u.db.WithContext(ctx).Model(&domain.User{}).Where("id = ?", userID)
